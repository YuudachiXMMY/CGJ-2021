...
default train2_examine_ske1 = False
default train2_examine_ske2 = False
default train2_examine_ske_bs = False
default train2_examine_box = False

default fi_train2_examine0 = 0
default fi_train2_examine1 = 0
default fi_train2_examine2 = 0
default fi_train2_examine_string = 0
default fi_train2_examine_train = 0
default fi_train2_examine_burngString = 0
default fi_train2_examine_ske1 = 0
default fi_train2_examine_ske2 = 0
default fi_train2_examine_ske_bs = 0
default fi_train2_examine_box = 0
...
screen train2():
    ...
    fixed:
        if first_train2:
            ...
            timer 3 action SetVariable('first_train2', False)
        if (curr_player_xpos > 500 and curr_player_xpos < 1050) and not train2_examine_train:
            text str("这是一辆运送煤炭的火车。"):
                ypos 75 xpos 200 color "#fff"
            timer 3 action SetVariable('train2_examine_train', True)
        if not train2_examine_string:
            if not train1_string:
                text str("断了一截的麻绳 或许可以拿来燃烧"):
                    ypos 75 xpos 200 color "#fff"
                timer 3 action SetVariable('train2_examine_string', True)
            else:
                text str("又是这根绳子"):
                    ypos 75 xpos 200 color "#fff"
                timer 3 action SetVariable('train2_examine_string', True)
        if train2_burnString and not train2_examine_burngString:
            text str("点燃了绳子 红色的火焰窜起起 我在它背后看到了几具实体的残骸 他们的结局与这节车厢一样，被大火吞噬。"):
                ypos 75 xpos 200 color "#fff"
            timer 3 action SetVariable('train2_examine_burngString', True)
        if not train2_examine_ske1 and not train2_burnString:
            text str("我阴影中看到了几具实体的残骸 他们的结局与这节车厢一样，被大火吞噬。"):
                ypos 75 xpos 200 color "#fff"
            timer 3 action SetVariable('train2_examine_ske1', True)
        if not train2_examine_ske1 and train2_burnString and not train2_examine_ske_bs:
            text str("轻轻翻动残骸，找到一个发黑的铁盒子，被锁住了。"):
                ypos 75 xpos 200 color "#fff"
            timer 3 action SetVariable('train2_examine_ske1', True), SetVariable('train2_examine_ske_bs', True), SetVariable('train2_box', True)
        if not train2_examine_ske2 and train2_box and not train2_examine_box and not train2_unlock:
            ...
            text str("慢慢打开盒子，里面装着几枚钱币，和几张身份证明，上面签名的字迹模糊不清。"):
                ypos 100 xpos 200 color "#fff"
            timer 3 action SetVariable('train2_examine_ske2', True),SetVariable('train2_examine_box', True)
    ...
    imagebutton:
        ...
        if not train2_burnString:
            ...
            action SetVariable('train2_examine_ske1', False), SetVariable('train2_box', True)
    ...
    if not train2_string:
        imagebutton:
            ...
            action [SetVariable("train2_string", True), SetVariable("string_number", string_number+1),
                Hide('mouseControl'),
                Hide('bgControl'),
